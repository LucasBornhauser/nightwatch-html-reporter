...
async.waterfall([
  ...
  function saveParsedXML(parsedData, next) {
    if (opts.debug.saveXML) {
      var fs = require('fs');
        var content  = JSON.stringify(parsedData, null, '\t');
        fs.writeFile(opts.debug.saveXML, content, function(err) {
        next(err, parsedData);
      });
    } else {
  ...
  if (err) return logger.error('Error generating report: ' + err.toString());

  logger.info('YYYYGenerated Report at: ' + reportFilename);

  if (opts.openBrowser) open(reportFilename);
